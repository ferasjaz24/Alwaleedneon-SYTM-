...
Text to translate: ${text}`;
  ...
  app.put("/api/payroll_runs/:id/employees/:empId", async (req, res) => {
    try {
      ...
      const updatedEmployees = employees.map((emp: any) => emp.id === empId ? { ...emp, ...req.body } : emp);
      await setDoc(ref, { employees: updatedEmployees }, { merge: true });
      res.json({ success: true });
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  ...
  // AI Commands Endpoint
  app.post("/api/ai-commands/execute", async (req, res) => {
    try {
      const { prompt, department, history } = req.body;
      if (!prompt) {
        return res.status(400).json({ error: "الرجاء إدخال الأمر النصي." });
      }

      // Fetch all employees and quotations to build context
      const employees = await getCollectionDocs("employees");
      const quotations = await getCollectionDocs("projects");
      const warehouseItems = await getCollectionDocs("warehouse_items");
      const journalEntries = await getCollectionDocs("journal_entries");

      // Condense context to save tokens and avoid noise
      const condensedEmployees = employees.map((e: any) => ({
        id: e.id,
        ...
        allowances: e.allowances || { housing: 0, transport: 0, food: 0 }
      }));

      const condensedQuotations = quotations.slice(0, 15).map((q: any) => ({
        id: q.id,
        clientName: q.clientName || q.client_name || "",
        projectTitle: q.projectTitle || q.project_title || "",
        dateCreated: q.dateCreated || q.date_created || "",
        expectedValue: Number(q.expectedValue || 0),
        stage: q.stage || "Draft"
      }));

      const condensedWarehouse = warehouseItems.slice(0, 15).map((w: any) => ({
        id: w.id,
        nameAr: w.nameAr || w.name_ar || "",
        nameEn: w.nameEn || w.name_en || "",
        qty: Number(w.qty || 0),
        unit: w.unit || "",
        cost: Number(w.cost || 0)
      }));

      const condensedJournals = journalEntries.slice(0, 5).map((j: any) => ({
        id: j.id,
        description: j.description || "",
        totalAmount: Number(j.totalAmount || j.total_amount || 0),
        date: j.date || ""
      }));

      const ai = await getGeminiClient();

      const systemPrompt = `أنت المساعد الذكي الفائق لبوابة الإدارة العليا (بوابة F24) لشركة فنون الوليد للصناعة.
مهمتك هي فهم الأوامر باللغة العربية وتحويلها إلى إجراءات قاعدة بيانات لتعديل أو إنشاء السجلات في قاعدة البيانات لمختلف قطاعات وأقسام الشركة الخمسة.

تاريخ اليوم: ${new Date().toISOString().split("T")[0]}
القسم المختار من واجهة المستخدم للتسهيل وتسريع البحث: ${department || "غير محدد"}

العمليات المتاحة للتنفيذ:
1. 'update_employee': تعديل حقول موظف حالي.
   - إذا كان الطلب تعديل راتب أو بدلات موظف، ابحث عن الاسم الأقرب في قائمة الموظفين (مثال: "محمد موسى" قد يطابق "محمد موسى العتيبي").
   - حدد 'employeeId' الصحيح.
   - ضع 'employeeUpdateData' والتي تحتوي على القيم التي سيتم تعديلها أو دمجها في ملف الموظف (مثل: basicSalary أو allowances.housing أو allowances.transport أو allowances.food).
2. 'create_employee': تسجيل موظف جديد تماماً.
   - يتم تفعيلها عند طلب إضافة موظف جديد أو تسجيل اسم موظف جديد (مثال: إذا طلب المستخدم إضافة الموظف "عمر" أو "عمر العتيبي").
   - يجب ملء 'employeeUpdateData' بالاسم والراتب والتفاصيل المحددة أو الافتراضية.
3. 'create_quotation': إنشاء عرض سعر جديد في مجموعة 'projects' (التي تسمى عروض الأسعار).
   - يجب أن تولد 'quotationData'.
   - تفاصيل 'quotationData':
     - clientName: اسم العميل أو الشركة (مثل "إيراب للمقاولات")
     - projectTitle: عنوان المشروع (مثل "مشروع توريد لوحات")
     - items: مصفوفة من العناصر، كل عنصر يحتوي على: { name, qty, price, total } حيث total هو qty * price.
     - expectedValue: مجموع العناصر بالكامل.
     - proposedPrice: مجموع العناصر بالكامل.
     - vatRate: 15 (افتراضي)
     - stage: 'Draft' (مسودة افتراضية)
     - salesRepName: 'المساعد الذكي F24'
4. 'update_quotation': تعديل حالة أو تفاصيل عرض سعر قائم.
   - يجب ملء 'quotationData' بالبيانات المعدلة مع توفير المعرف 'quotationId' كحقل 'quotationId'.
5. 'create_procurement_request': إنشاء طلب شراء جديد في مجموعة 'pricing_studies' أو طلبات المشتريات.
   - يجب ملء 'quotationData' بالتفاصيل المطلوبة.
6. 'update_supplier_price': تعديل سعر توريد صنف في المستودع.
   - يجب ملء 'quotationData' متضمناً 'itemId' و 'cost'.
7. 'create_journal_entry': تسجيل قيد محاسبي جديد في مجموعة 'journal_entries'.
   - يجب ملء 'quotationData' بالتفاصيل والمبلغ الإجمالي والوصف.
8. 'update_production_status': تحديث حالة التصنيع والتشغيل لمشروع أو أمر إنتاج.
   - يجب ملء 'quotationData' بالمعرف 'projectId' وحالة الإنتاج 'stage'.
9. 'update_warehouse_stock': زيادة أو نقصان كمية المواد أو المنتجات في المستودع 'warehouse_items'.
   - يجب ملء 'quotationData' بـ 'itemId' ومقدار التغيير في الكمية 'qtyChange' (موجب أو سالب).

قوانين الاستجابة (JSON strictly):
- إذا كان الأمر مبهماً، أو لم تجد الموظف المطلوب، أو هناك تفاصيل ناقصة للعملية، اضبط status="clarify" واكتب سؤالاً واضحاً ومحترماً باللغة العربية في 'message' يطلب التفاصيل الناقصة.
- إذا كانت التفاصيل كاملة ومؤكدة، اضبط status="execute" واملأ الإجراء 'action' واكتب ملخصاً جميلاً باللغة العربية في 'message' يوضح ما قمت به بنجاح تام.

سجلات وموظفو الموارد البشرية الحالية في النظام:
${JSON.stringify(condensedEmployees, null, 2)}

قائمة عروض الأسعار الأخيرة في النظام:
${JSON.stringify(condensedQuotations, null, 2)}

قائمة أصناف مستودع المواد الأولية والإنتاج:
${JSON.stringify(condensedWarehouse, null, 2)}

قائمة القيود والعمليات المالية الحديثة:
${JSON.stringify(condensedJournals, null, 2)}
...
${JSON.stringify(history || [])}`;
      ...
      const response = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: [
          { role: "user", parts: [{ text: `${systemPrompt}\n\nالأمر الجديد من المستخدم: ${prompt}` }] }
        ],
        config: {
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              status: { type: Type.STRING, description: "Either 'execute' or 'clarify'" },
              message: { type: Type.STRING, description: "The response explanation in Arabic" },
              suggestedDepartment: { type: Type.STRING, description: "Suggested department: 'employees', 'quotations' or 'none'" },
              action: {
                type: Type.OBJECT,
                properties: {
                  type: { type: Type.STRING, description: "Action type: 'update_employee', 'create_employee', 'create_quotation', 'update_quotation', 'create_procurement_request', 'update_supplier_price', 'create_journal_entry', 'update_production_status', 'update_warehouse_stock'" },
                  employeeId: { type: Type.STRING },
                  employeeUpdateData: { type: Type.OBJECT },
                  quotationId: { type: Type.STRING },
                  quotationData: { type: Type.OBJECT }
                }
              }
            },
      ...
      // If execution is authorized, execute it immediately on Firestore!
      if (parsed.status === "execute" && parsed.action) {
        const { type, employeeId, employeeUpdateData, quotationId, quotationData } = parsed.action;

        if (type === "update_employee" && employeeId && employeeUpdateData) {
          const ref = doc(db, "employees", employeeId);
          const snap = await getDoc(ref);
          if (snap.exists()) {
            const existing = snap.data();
            const merged = { ...existing, ...employeeUpdateData };
            // Ensure nested allowances merge nicely
            if (employeeUpdateData.allowances && existing.allowances) {
              merged.allowances = { ...existing.allowances, ...employeeUpdateData.allowances };
            }
            ...
            await setDoc(doc(db, "audit_logs", logId), {
              id: logId,
              action: "تعديل موظف بالذكاء",
              timestamp: new Date().toISOString(),
              operator_id: "F24_AI_AGENT",
              notes: `تم تعديل بيانات الموظف ${existing.arabicName || employeeId} آلياً: ${parsed.message}`
            });
          } else {
            parsed.status = "clarify";
            parsed.message = `عذراً، لم أتمكن من العثور على الموظف ذو المعرف ${employeeId} في قاعدة البيانات لتحديث بياناته.`;
          }
        } else if (type === "create_employee" && employeeUpdateData) {
          const empId = `EMP-${Math.floor(1000 + Math.random() * 9000)}`;
          const completeEmp = {
            id: empId,
            firestoreId: empId,
            company: "شركة فنون الوليد",
            arabicName: employeeUpdateData.arabicName || "موظف جديد",
            englishName: employeeUpdateData.englishName || "New Employee",
            jobTitle: employeeUpdateData.jobTitle || "عامل",
            department: employeeUpdateData.department || "Neon Fabrication",
            basicSalary: Number(employeeUpdateData.basicSalary || 1500),
            allowances: {
              housing: Number(employeeUpdateData.allowances?.housing || 300),
              transport: Number(employeeUpdateData.allowances?.transport || 0),
              food: Number(employeeUpdateData.allowances?.food || 0),
              otherAllowances: Number(employeeUpdateData.allowances?.otherAllowances || 0),
              deductions: 0,
              loans: 0,
              status: "Active"
            },
            nationality: employeeUpdateData.nationality || "سعودي",
            iqamaId: employeeUpdateData.iqamaId || String(Math.floor(1000000000 + Math.random() * 9000000000)),
            dateOfJoining: employeeUpdateData.dateOfJoining || new Date().toISOString().split("T")[0],
            birthDate: employeeUpdateData.birthDate || "1995-01-01"
          };
          await setDoc(doc(db, "employees", empId), completeEmp);

          // Add audit log
          const logId = `AUD-${Date.now()}`;
          await setDoc(doc(db, "audit_logs", logId), {
            id: logId,
            action: "إضافة موظف بالذكاء",
            timestamp: new Date().toISOString(),
            operator_id: "F24_AI_AGENT",
            notes: `تم تسجيل الموظف الجديد ${completeEmp.arabicName} برقم ${empId} آلياً: ${parsed.message}`
          });

          parsed.message = `✅ تم تسجيل الموظف الجديد بنجاح برقم: ${empId}\n\n${parsed.message}`;
        } else if (type === "create_quotation" && quotationData) {
          const id = `QT-2026-${Math.floor(100 + Math.random() * 900)}`;
          const completeQuo = {
            id,
            stage: "Draft",
            vatRate: 15,
          ...
          await setDoc(doc(db, "projects", id), completeQuo);

          // Add audit log
          const logId = `AUD-${Date.now()}`;
          await setDoc(doc(db, "audit_logs", logId), {
            id: logId,
            action: "إنشاء عرض سعر بالذكاء",
            timestamp: new Date().toISOString(),
            operator_id: "F24_AI_AGENT",
            notes: `تم إنشاء عرض سعر للعميل ${completeQuo.clientName || ""} آلياً: ${parsed.message}`
          });

          parsed.message = `✅ تم حفظ وتثبيت السند في النظام برقم: ${id}\n\n${parsed.message}`;
        } else if (type === "update_quotation" && quotationId && quotationData) {
          const ref = doc(db, "projects", quotationId);
          const snap = await getDoc(ref);
          if (snap.exists()) {
            await setDoc(ref, { ...snap.data(), ...quotationData }, { merge: true });
          }
        } else if (type === "create_procurement_request" && quotationData) {
          const id = `PR-${Date.now().toString().slice(-4)}`;
          await setDoc(doc(db, "pricing_studies", id), {
            id,
            timestamp: new Date().toISOString(),
            ...quotationData
          });
        } else if (type === "update_supplier_price" && quotationData && quotationData.itemId) {
          const ref = doc(db, "warehouse_items", quotationData.itemId);
          const snap = await getDoc(ref);
          if (snap.exists()) {
            await setDoc(ref, { ...snap.data(), cost: Number(quotationData.cost) }, { merge: true });
          }
        } else if (type === "create_journal_entry" && quotationData) {
          const id = `JE-${Date.now().toString().slice(-4)}`;
          await setDoc(doc(db, "journal_entries", id), {
            id,
            date: new Date().toISOString().split("T")[0],
            ...quotationData
          });
        } else if (type === "update_production_status" && quotationId && quotationData) {
          const ref = doc(db, "projects", quotationId);
          const snap = await getDoc(ref);
          if (snap.exists()) {
            await setDoc(ref, { ...snap.data(), stage: quotationData.stage || "Production" }, { merge: true });
          }
        } else if (type === "update_warehouse_stock" && quotationData && quotationData.itemId) {
          const ref = doc(db, "warehouse_items", quotationData.itemId);
          const snap = await getDoc(ref);
          if (snap.exists()) {
            const existing = snap.data();
            const newQty = (existing.qty || 0) + Number(quotationData.qtyChange || 0);
            await setDoc(ref, { ...existing, qty: newQty }, { merge: true });
          }
        }
      }
      ...
      res.status(500).json({ error: err.message || "حدث خطأ أثناء معالجة أمر المساعد الذكي." });
    }
  });
